		...
		struct pwm_device *pwm, int duty_ns, int period_ns) 
{
	...
	//printk("fpga_pwm_config—— reg_pulsewidth:reg=%x addr=%x  value=%x\n", (u32)fpgapwm->fpga_pwm_chans[pwm->hwpwm].reg_pulsewidth, (u32)(fpgapwm->mmio_base + fpgapwm->fpga_pwm_chans[pwm->hwpwm].reg_pulsewidth), value);
        if (fpgapwm->cominterface == MOTION_EIM_INTERFACE) {
	    rs = fpga_writel(value, fpgapwm->mmio_base + fpgapwm->fpga_pwm_chans[pwm->hwpwm].reg_pulsewidth); 

	}else if(fpgapwm->cominterface == MOTION_SPI_INTERFACE){
	    rs = fpgax_writel(value, fpgapwm->mmio_base + fpgapwm->fpga_pwm_chans[pwm->hwpwm].reg_pulsewidth); 
	}
	if (rs)
		return rs;
...
static void fpga_pwm_disable(struct pwm_chip *chip, struct pwm_device *pwm)
{
	...
	cellmask = fpgapwm->fpga_pwm_chans[pwm->hwpwm].cell_mask;
	    ...
	    rs = fpgax_update_lbits(fpgapwm->mmio_base + fpgapwm->ctrl_reg, cellmask, 0);
	}
}
...
static int fpga_pwm_probe(struct platform_device *pdev)
{
	...
	if (ret >= 0){
		dev_dbg(&pdev->dev,"fpgabus:%x\n", fpgapwm->cominterface);
                fpgapwm->cominterface = MOTION_EIM_INTERFACE;
		...
		if (ret >= 0){
			dev_dbg(&pdev->dev, "fpgabus:%x\n", fpgapwm->cominterface);
                        fpgapwm->cominterface = MOTION_SPI_INTERFACE;
                        printk("motordev->cominterface = MOTION_SPI_INTERFACE\n");
                } 
                else{
		    printk("error fpgabus\n");
                    return -EINVAL;
		}
	}
	...
	if (ret){
        ...
        if (fpgapwm->cominterface == MOTION_EIM_INTERFACE) {
            ...
            fpgapwm->mmio_base = fpgax_io_get(fpga_cs);
        }
        else{
	    printk("error fpgabus1\n");
            return -ENOMEM;
        }
