...
int photosensor_get_trigger_next(struct photosensor *sensor, struct sensor_trigger *trigger)
{
	...
	if (sensor->type == PHOTOSENSOR_DIGITAL) {
		...
		fpga_regs = fpgax_io_get(PHOTOSENSOR_CS) + FPGA_REG_SENSOR_DC_SENSOR_MASK;
		fpgax_readl(&val, fpga_regs);
		...
		fpga_regs = fpgax_io_get(PHOTOSENSOR_CS) + FPGA_REG_SENSOR_DC_COMPARE_MODE;
		fpgax_readl(&val, fpga_regs);
		if (val & (1<<sensor->dig_bit_index)) {
			trigger->mode = 1;
		}
