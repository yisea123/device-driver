...
static ssize_t synctest_err_show(struct device *dev, struct device_attribute *attr, char *buf );
...
static ssize_t downloadtest_show(struct device *dev, struct device_attribute *attr, char *buf );
static ssize_t downloadtest_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t count);

extern int download_proc(const struct firmware *fw, const char *fw_name);
...
static DEVICE_ATTR(downloadtest, S_IRUGO | S_IWUSR, downloadtest_show, downloadtest_set);
...
static DEVICE_ATTR(synctest_err, S_IRUGO, synctest_err_show, NULL);
...
u32 sample_buf[10*SCANLINE_DATA_CNT];
...
{
	size_t count = 0;
	u32 light_sel, cnt, offset = 0, imgdata_status;// timeout =  msecs_to_jiffies(2000);
	int i, lights;
...
{
	size_t count = 0;
	memcpy(buf, imagebuf, SCANLINE_DATA_CNT); 
	count = SCANLINE_DATA_CNT;
	return count;
}
...
static ssize_t synctest_show(struct device *dev, struct device_attribute *attr, char *buf )
{
	...
	{
#define CRC32_VAL 0xDFAFAA1A
		memcpy(imagebuf, imgdata, SCANLINE_DATA_CNT*10);
		crc32 = crc32_le(0, (char *)imagebuf, SCANLINE_DATA_CNT*10);
//		printk("crc32 = %08X\n", crc32);
		if(crc32 != CRC32_VAL)
		{
			int i;
			...
			for (i=0; i < 1008*10; i++) {
				if (imagebuf[i] != sample_buf[i]) {
					printk("%d, corect=%08x<-err=%08x\n", i, sample_buf[i], imagebuf[i]);
					dcnt++;
				}
				if (dcnt > 100) {
					break;
...
static irqreturn_t fpga_debug_isr3(int irq, void *dev_id)
{
	printk("fpga int 3 handled\n");
	return IRQ_HANDLED;
}
...
static int fpga_debug_probe(struct platform_device *pdev)
{
	...
	if(request_firmware(&fw, "sample.dat", &ghost_device))
	{
	    printk("request_firmware err!\n");
	    return -ENOENT;
	}
	printk("\n size : %d\n", fw->size);
	memcpy(sample_buf, (const void *)fw->data, fw->size);
	release_firmware(fw);
	return 0;
}
...
{
//	fpga_request_irq(3, fpga_debug_isr3, NULL);
