...
static int fpga_motor_probe(struct platform_device *pdev)
{
	...
	dev_dbg(&pdev->dev, "fpga_cs of FPGA motor interrupt registers is %d\n", fpga_cs);
	if (IS_ERR_VALUE(ret)) {
		dev_err(&pdev->dev, "Failed to parse chip select number of FPGA motor interrupt registers\n");
		return ret;
	}

	reg_base = fpga_io_get(fpga_cs) + 0x1000;
	if (IS_ERR(reg_base)) {
