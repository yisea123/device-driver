...
int steppermotor_check_config(struct steppermotor *motor, const struct steppermotor_config *config)
{
	...
	{
		dev_err(motor->dev, "steppermotor_check_config steps_to_run\n");
		return -EINVAL;
	}
	...
	for (i=0; i<config->num_speed; i++)
	{
		if (!speedinfo)
		{
			dev_err(motor->dev, "steppermotor_check_config1@@\n");
			return -EINVAL;
		}
		...
		{
			dev_err(motor->dev, "steppermotor_check_config2\n");
			return -EINVAL;
		}
	...
	{
		dev_err(motor->dev, "steppermotor_check_config3\n");
		return -EINVAL;
	}
	...
	{
		dev_err(motor->dev, "steppermotor_check_config4\n");
		return -EINVAL;
	}
	...
	{
		dev_err(motor->dev, "steppermotor_check_config5\n");
		return -EINVAL;
	}
	steps -= motor->feature.speeds[index1].accel_steps;
	if (steps <= 0)
	{
		dev_err(motor->dev, "steppermotor_check_config6\n");
		return -EINVAL;
	}
	...
	if (config->num_speed == 1)	// single-step configuration
	{
		...
		if (steps <= 0)
		{
			dev_err(motor->dev, "steppermotor_check_config7\n");
			return -EINVAL;
		}
	}
	else
	{
		...
		{
			dev_err(motor->dev, "steppermotor_check_config8\n");
			return -EINVAL;
		}
		steps -= motor->feature.speedshifts[index2].steps;
		if (steps <= 0)
		{
			dev_err(motor->dev, "steppermotor_check_config9\n");
			return -EINVAL;
		}
		...
		if (IS_ERR_VALUE(index1))
		{
			dev_err(motor->dev, "steppermotor_check_config10\n");
			return -EINVAL;
		}
		steps -= motor->feature.speeds[index2].decel_steps;
		if (steps <= 0)
		{
			dev_err(motor->dev, "steppermotor_check_config11\n");
			return -EINVAL;
		}
	}
...
int steppermotor_set_scanstart_steps(struct steppermotor *motor, int steps)
{
	if (motor && motor->ops->set_skew_steps)
		return motor->ops->set_scanstart_steps(motor, steps);

	return -EINVAL;
}
EXPORT_SYMBOL_GPL(steppermotor_set_scanstart_steps);
