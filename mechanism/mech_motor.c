	...
	struct motor_data *pmotor_data,unsigned char dir,  motor_mov_t  * pmotor_mov)		
{
	...
	#if defined(MECH_TRIGER_STEP_SCAN) || defined(MECH_TRIGER_SCAN_MODE)
	if(pmotor_data->pmotor_mov->scantriger_source)
	{
		#if defined(MECH_TRIGER_SCAN_MODE)
		if (pmotor_data->pmotor_mov->scantriger_source.scantriger_mode==SCANTRIGER_MODE_AUTO)
		#endif
		{
			pr_debug("step_motor_start:steps_to_scan=%d\n", pmotor_data->pmotor_mov->scantriger_source->steps_to_scan);
			ret = steppermotor_set_scanstart_steps(pmotor_data->motor_dev.psteppermotor, pmotor_data->pmotor_mov->scantriger_source->steps_to_scan);
			if (ret) {
				pr_debug("step_motor_start:steppermotor_set_scanstart_steps error!\n");
				ret = -RESN_MECH_ERR_MOTOR_MOVE_SET_CONFIG_ERR;
				motormove_err_callback(pmotor_data, ret);
				return ret;
			}
...
static int step_motor_wait_stop(struct motor_data *pmotor_data)
{
	...
	if(ret)
	{
		 printk("step_motor_wait_stop:step_motor_start:sensor_clear_trigger_next error!\n");
...
int motor_get_running_steps(mechanism_uint_motor_data_t *punit_motor_data, unsigned short motor_mask, int *psteps)
{
	...
	pr_debug("1.motor_get_running_steps\n");
	motor_get_data(punit_motor_data, motor_mask, pmotor_data, i);
	if (i==punit_motor_data->motor_num) {
		return -RESN_MECH_ERR_MOTOR_GETDATA;
	}
    	
	pr_debug("2.motor_get_running_steps\n");
	switch(pmotor_data->motor_type)
	{
	case MOTOR_STEP_TYPE:
